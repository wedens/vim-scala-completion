...
class CompletionTypeDetectorSpec extends Specification {
  ...
  "completion type detector" should {
    ...
    "detect scope completion on blank line" in  {
      ...
      detector.detect(line, line.length - 1) must_== CompletionTypes.Scope
    }

    "detect scope completion after ;" in {
      val line = "str.toString(); "

      detector.detect(line, line.indexOf(" ")) must_== CompletionTypes.Scope
    }
    ...
    "detect scope completion iside of []" in {
      ...
      detector.detect(line, line.indexOf(",") + 2) must_== CompletionTypes.Scope
    }
    ...
    "detect scope in between extends statement and other code" in {
      ...
      detector.detect(line, line.indexOf("extends") + 2) must_== CompletionTypes.Scope
    }
    ...
    "not detect completion after dot inside string" in {
       ...
       detector.detect(line, line.indexOf(".")) must_== CompletionTypes.NoCompletion
    }
    ...
    "detect scope completion in between if in case statement and =>" in {
      ...
      detector.detect(line, line.indexOf("if") + 2) must_== CompletionTypes.Scope
    ...
    "detect type completion in import with curly braces" in {
      ...
      detector.detect(line, line.length - 2) must_== CompletionTypes.Type
    }
    ...
    "detect type completion with out of bound position" in {
      ...
      detector.detect(line, line.length) must_== CompletionTypes.Type
    }
